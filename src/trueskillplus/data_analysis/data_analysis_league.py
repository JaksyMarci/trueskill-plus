...
df = df[df['League'] == 'NALCS'].reset_index(drop=True)  # filter DF
...
ts_plus_env = trueskillplus.Trueskillplus(
    draw_probability=0, stat_coeff=0.000075)
...
for index, row in df.iterrows():
    b_win_prob = ts_plus_env.win_probability([ts_ratings[row['blueTeamTag']]], [
                                             ts_ratings[row['redTeamTag']]])

    df.at[index, 'predicted_bResult'] = 1 if b_win_prob > 0.5 else 0

    if row['bResult'] == 1:
        # blue win
        b_new_rating, r_new_rating = trueskill.rate_1vs1(
            ts_ratings[row['blueTeamTag']], ts_ratings[row['redTeamTag']])
        ts_ratings[row['blueTeamTag']] = b_new_rating
        ts_ratings[row['redTeamTag']] = r_new_rating
    else:
        # red win
        r_new_rating, b_new_rating = trueskill.rate_1vs1(
            ts_ratings[row['redTeamTag']], ts_ratings[row['blueTeamTag']])
        ts_ratings[row['redTeamTag']] = r_new_rating
        ts_ratings[row['blueTeamTag']] = b_new_rating
...
for index, row in df.iterrows():
    b_win_prob = ts_plus_env.win_probability([ts_plus_ratings[row['blueTeamTag']]], [
                                             ts_plus_ratings[row['redTeamTag']]])

    df.at[index, 'predicted_bResult'] = 1 if b_win_prob > 0.5 else 0

    pred_array = np.array(row[['bResult', 'rResult', 'bKills', 'bTowers', 'bInhibs', 'bDragons', 'bBarons',
